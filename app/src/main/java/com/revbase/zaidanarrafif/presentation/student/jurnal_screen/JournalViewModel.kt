...
class JournalViewModel @Inject constructor(
    ...
    fun createDailyJournal(kegiatan: List<Int>){
        createDailyJournalUseCase("Bearer $_token",kegiatan,_nis ).onEach { result->
            when(result){
                is Resource.Success->{
                    _ibadahJournalState.value = JournalState(journal = result.data)
                }
                is Resource.Error->{
                    _ibadahJournalState.value = JournalState(error = result.message?:"Terjadi kesalahan tidak terduga")
                }
                is Resource.Loading->{
                    _ibadahJournalState.value = JournalState(isLoading = true)
                }
            }
        }.launchIn(viewModelScope)

...
class GetTodaysJournalUseCase @Inject constructor(
    ...
    operator  fun invoke(token:String,nis:Int, jenis:String?):Flow<Resource<Journal>> = flow {
        try {

            emit(Resource.Loading<Journal>())
            val  response = repository.getTodaysJournal(token,nis,jenis)

            if(response.data != null){
                val todaysJournal = response.data.toJournal()
